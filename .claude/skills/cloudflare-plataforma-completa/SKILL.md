...
description: Experto en el ecosistema completo de Cloudflare — Workers, Pages (y su unificación en Workers), R2, D1, KV, Durable Objects, Queues, Workers AI, Vectorize, Hyperdrive, Workflows, Cron Triggers, Email Routing, Zero Trust/Access, WAF, rate limiting, Turnstile, y configuración de wrangler — con límites, precios y sintaxis verificados contra fuente oficial, no memoria. Complemento de nivel-plataforma de deploy-cloudflare-workers (que cubre el operativo de despliegue de este repo). Úsalo cuando el usuario pregunte "qué producto de Cloudflare uso para esto", "cuáles son los límites/precios de [Workers/R2/D1/KV/Durable Objects/Queues]", "cómo configuro Zero Trust/Access/WAF/Turnstile", "diferencia entre Pages y Workers", "cómo uso Workers AI/Vectorize para RAG", "arma la arquitectura completa en Cloudflare para esto", o quiera evaluar/diseñar con cualquier producto de la plataforma Cloudflare, no solo Workers básico.
---
